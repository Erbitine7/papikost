            ...
            <?php if (! empty($computers) && is_array($computers)): ?>
                ...
                <?php foreach ($computers as $computer): ?>
                    <tr class="hover:bg-slate-800 border-b border-slate-700">
                        ...
                        <td class="py-4 px-2"><?= date('d/m/Y', strtotime($computer['updated_at'] ?? $computer['created_at'] ?? 'now')) ?></td>
                        <td class="pt-2 px-3 font-semibold text-right flex justify-end">
                            <a href="<?= base_url('computer/detail?id=' . $computer['id']) ?>" class="rounded-lg bg-lime-500 pt-1 pb-2 px-2 ml-2 text-slate-900 hover:bg-lime-600 flex"><ion-icon class="pt-1 text-2xl" name="eye-outline"></ion-icon></a>
