    ...
    <table class="outline outline-slate-700 rounded-sm w-full text-left">
        ...
        <tbody>
            ...
            <tr class="hover:bg-slate-800 border-b border-slate-700">
                ...
                <td class="p-4"><?= esc($computer['pc_category'] ?? ''); ?></td>
                <td class="p-4">Rp <?= number_format($computer['tariff'] ?? 0, 0, ',', '.'); ?></td>
                <td class="py-4 px-2"><?= esc($computer['status']); ?></td>
                <td class="py-4 px-2"><?= $computer['created_at']; ?></td>
                <td class="py-4 px-2"><?= $computer['updated_at']; ?></td>
                <td class="pt-2 px-3 font-semibold text-right flex justify-end">
                    <a href="<?= base_url('computer/edit?id=' . $computer['id']) ?>" class="rounded-lg bg-yellow-400 pt-1 pb-2 px-2 ml-2 text-slate-900 hover:bg-yellow-600 flex"><ion-icon class="pt-1 text-2xl" name="pencil-outline"></ion-icon></a>
                    <a onclick="openDelete(<?= $computer['id'] ?>)" class="rounded-lg bg-red-500 pt-1 pb-2 px-2 ml-2 hover:bg-red-700 hover:text-gray-300 flex"><ion-icon class="pt-1 text-2xl" name="trash-outline"></ion-icon></a>
                </td>
            </tr>
            <?php endforeach; ?>
...
<dialog id="dialog-delete" class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 open:flex backdrop:bg-slate-950/[80%]">
    <form action="#" method="POST" class="bg-slate-800 w-75 shadow-lg rounded-2xl flex flex-col py-4 px-5">
        ...
        <div class="flex flex-row w-full items-center justify-center">
            ...
            <input readonly type="number" name="pcno" id="del" class="w-    2 bg-slate-900 p-2 rounded-md text-center ml-2 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none">
        </div>
